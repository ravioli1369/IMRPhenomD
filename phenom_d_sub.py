...
def subtract3PNSS(Mf, mass1, mass2, eta, eta2, xi, chi1, chi2):
    M = mass1 + mass2
    eta = mass1 * mass2 / M / M
    m1byM = mass1 / M
    m2byM = mass2 / M
...
def phenom_d_inspiral_phase(Mf, mass_1, mass_2, eta, eta2, xi, chi1, chi2):
    ...
    pn_ss3, Dpn_ss3 = subtract3PNSS(Mf, mass_1, mass_2, eta, eta2, xi, chi1, chi2)
    ins_phasing -= pn_ss3
...
def phenom_d_phase(Mf, mass_1, mass_2, eta, eta2, chi1, chi2, xi):
    ...
    int_phase_f1, int_Dphase_f1 = phenom_d_int_phase(PHI_fJoin_INS, eta, eta2, xi)
    ...
    mrd_phase_rd, mrd_Dphase_rd = phenom_d_mrd_phase(fRDJoin, eta, eta2, chi1, chi2, xi)
    ...
    theta_minus_f2 = torch.heaviside(fRDJoin - Mf, torch.tensor(0.0, device=Mf.device))
    theta_plus_f2 = torch.heaviside(Mf - fRDJoin, torch.tensor(1.0, device=Mf.device))
...
def phenom_d_int_amp(Mf, eta, eta2, Seta, chi1, chi2, chi12, chi22, xi):
    ...
    v1, d1 = phenom_d_inspiral_amp(Mf1, eta, eta2, Seta, xi, chi1, chi2, chi12, chi22)
...
def phenom_d_amp(
    ...
    theta_minus_f2 = torch.heaviside(Mf_peak - Mf, torch.tensor(0.0, device=Mf.device))
    theta_plus_f2 = torch.heaviside(Mf - Mf_peak, torch.tensor(1.0, device=Mf.device))
...
def phenom_d_htilde(
    ...
    Psi_ref, _ = phenom_d_phase(Mf_ref, mass_1, mass_2, eta, eta2, chi1, chi2, xi)
